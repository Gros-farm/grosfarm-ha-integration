...
def build_lamp_on_payload(state: Any) -> dict[str, Any]:
    """Service-data для включения лампы «на полную»: яркость 100% + белый.

    Белый — насколько поддерживает устройство; для switch/onoff — пустой dict.

    Спектр диммируемой лампы (WLED и т.п.) намеренно прижимаем к белому на
    максимуме: DLI-controller калибрует lamp_ppfd под состояние «лампа на
    полную», поэтому включать надо детерминированно, а не «как было».
    """
    modes = {str(m) for m in (state.attributes.get("supported_color_modes") or [])}
    data: dict[str, Any] = {}
    if modes - {"onoff"}:  # всё кроме чистого onoff умеет яркость
        data["brightness_pct"] = 100
    if "rgbww" in modes:
        data["rgbww_color"] = [0, 0, 0, 255, 255]
    elif "rgbw" in modes:
        data["rgbw_color"] = [0, 0, 0, 255]
    elif "rgb" in modes:
...
async def async_apply_lamp(hass: HomeAssistant, entity_id: str, on: bool) -> None:
    """Вкл/выкл лампу с учётом домена.

    light.* → light.turn_on (полный белый при on) / light.turn_off.
    switch.* (и прочее) → switch.turn_on / switch.turn_off.
    Бросает наружу — вызывающий решает, что делать со статусом.
    """
    if entity_id.split(".", 1)[0] == "light":
        if on:
            st = hass.states.get(entity_id)
            payload = build_lamp_on_payload(st) if st is not None else {}
            await hass.services.async_call(
                "light", "turn_on", {"entity_id": entity_id, **payload}, blocking=True
            )
        else:
            await hass.services.async_call(
...
class GrosfarmLightController:
    ...
    async def _ensure_lamp(self, state: bool) -> None:
        ...
        # Уже в нужном состоянии И это мы его выставили — выходим. Проверка на
        # _lamp_state важна для light.*: если лампу включили снаружи (или это наш
        # самый первый on), нужно один раз прогнать turn_on с яркостью/цветом —
        # иначе она не окажется на «полную белую». Для switch это просто no-op.
        if current_on == state and self._lamp_state == state:
            return
        try:
            await async_apply_lamp(self.hass, eid, state)
            self._lamp_state = state
        except Exception:
            _LOGGER.exception(
                "ensure_lamp failed (%s → %s)", eid, "on" if state else "off"
            )
            self._status = STATUS_LAMP_UNAVAIL
